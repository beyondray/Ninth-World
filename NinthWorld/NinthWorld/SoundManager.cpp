...
#define DISTANCE_FACTOR       48.0
#define ROLLOFF_SCALE         0.5



template<> SoundManager* Singleton<SoundManager>::msSingleton = 0;
...
void SoundManager::PlaySound(int soundIndex, SceneNode *soundNode, int *channelIndex,float volume/* =1.0f */)
   {
...
//判断声道是否正在播放
bool SoundManager::isPlaying(int* channelIndex, FMOD::Channel* soundChannel)
{
	if (*channelIndex == INVALID_SOUND_CHANNEL)
		return false;
	...
	system->getChannel(*channelIndex, &soundChannel);
	bool isPlaying;
	FMOD_RESULT result = soundChannel->isPlaying(&isPlaying);
...
//得到音量
bool SoundManager::getVolume(float* volume, int *channelIndex)
{
	FMOD::Channel* channel;
...
//暂停声音
bool SoundManager::pauseSound(int *channelIndex)
{
	FMOD::Channel* channel;
	if (!channelIndex || *channelIndex == INVALID_SOUND_CHANNEL)
		return false;
